...
K.set_image_data_format("channels_last")
 ...
 #u-net model
class Unet_model_simple(object):
    ...
    def unet(self,inputs, nb_classes=4, start_ch=64, depth=3, inc_rate=2. ,activation='relu', dropout=0.2, batchnorm=True, upconv=True,format_='channels_last'):
        ...
        """
        o = self.level_block_simple(inputs,start_ch, depth, inc_rate,activation, dropout, batchnorm, upconv,format_)
    ...
    def level_block_simple(self,m, dim, depth, inc, acti, do, bn, up,format_="channels_last"):
        if depth > 0:
            n = self.res_block_enc_simple(m,0.0,dim,acti, bn,format_)
            #using strided 2D conv for donwsampling
            m = Conv2D(int(inc*dim), 2,strides=2, padding='same',data_format = format_)(n)
            m = self.level_block_simple(m,int(inc*dim), depth-1, inc, acti, do, bn, up )
            if up:
    ...
    def res_block_enc_simple(self,m, drpout,dim,acti, bn,format_="channels_last"):
    ...
    def res_block_dec_simple(self,m, drpout,dim,acti, bn,format_="channels_last"):
